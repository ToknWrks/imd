 ...
 *   - v1 removed: legacy records are refused (never overwritten), their keys ignored.
 */
...
test("v2 wallet lifecycle: derive → copilot-null → read → browser-activate → v1 refused", () => {
  ...
  assert.match(out, /PASS v1-record-refused/);
  assert.match(out, /PASS v1-key-ignored/);
  assert.match(out, /PASS v2-key-resolves/);
  assert.match(out, /PASS v1-api-gone/);
